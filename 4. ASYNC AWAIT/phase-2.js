...
async function workout(totalTime) {
  try {
    const stretching = await stretch(totalTime)
    const threadmill = await runOnTreadmill(stretching)
    const weights = await liftWeights(threadmill)
    console.log(`done working out with ${weights/1000} seconds left`)
  } catch (error) {
